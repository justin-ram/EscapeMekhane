...
namespace Pinwheel.VistaEditor
{
    ...
    public class DetailTemplateInspector : Editor
    {
        ...
        private static readonly GUIContent GRASS_SHAPE = new GUIContent("Grass Shape", "Select the mesh used for rendering texture based grass, in Polaris terrain.");
        ...
        public override void OnInspectorGUI()
        {
            ...
            DetailTemplate.TextureBasedGrassShape grassShape = m_instance.textureBasedGrassShape;
            if (renderMode == DetailRenderMode.Grass || renderMode == DetailRenderMode.GrassBillboard)
            {
#if GRIFFIN
                if (displayMode == DisplayMode.Polaris || displayMode == DisplayMode.All)
                {
                    grassShape = (DetailTemplate.TextureBasedGrassShape)EditorGUILayout.EnumPopup(GRASS_SHAPE, m_instance.textureBasedGrassShape);
                }
#endif
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                m_instance.textureBasedGrassShape = grassShape;
            ...
            if (!EditorCommon.IsPersonalEdition() &&
                (EditorCommon.HasAssignedObjectReference(serializedObject.FindProperty("m_textureVariants")) ||
                 EditorCommon.HasAssignedObjectReference(serializedObject.FindProperty("m_prefabVariants"))))
            {
                EditorGUILayout.HelpBox(
                    "Detail variants are retained in this asset but no longer used during terrain population. Create one Detail Template per texture or prefab. In the graph, use Split Mask before Detail Density Output for Unity terrain, or Split Positions before Detail Instance Output for Polaris terrain.",
                    MessageType.Warning);
            }
            ...
            if (EditorGUILayout.LinkButton(VEGETATION_ASSETS_AFF_TEXT))
            {
                NetUtils.TrackClick("vegetation-aff", UILocation.Inspector_DetailTemplate);
