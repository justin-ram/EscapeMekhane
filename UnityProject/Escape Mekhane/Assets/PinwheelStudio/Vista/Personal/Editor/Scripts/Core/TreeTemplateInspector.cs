        ...
        public override void OnInspectorGUI()
        {
                ...
                EditorCommon.HasAssignedObjectReference(serializedObject.FindProperty("m_prefabVariants")))
            {
                EditorGUILayout.HelpBox(
                    "Tree prefab variants are retained in this asset but no longer used during terrain population. Create one Tree Template per prefab, then use Split Positions in the graph to distribute positions between their Tree Output nodes.",
                    MessageType.Warning);
            }
            ...
            {
                NetUtils.TrackClick("vegetation-aff", UILocation.Inspector_TreeTemplate);
