...
namespace Pinwheel.VistaEditor
{
    ...
    public class VistaManagerInspector : Editor
    {
        ...
        {
            if (EditorCommon.BeginFoldout(GeneralGUI.ID, GeneralGUI.HEADER, null, false))
            {
            ...
            if (EditorCommon.BeginFoldout(AddTilesGUI.ID, AddTilesGUI.HEADER, null, true))
            {
                ...
                foreach (ITerrainSystem s in terrainSystems)
                {
                    if (GUILayout.Button($"Add All {s.terrainLabel}s"))
                    {
                        ...
                        Object[] terrainObjects = Utilities.FindObjectsNoSortCompat<Component>()
                            .Where(c => c != null && terrainComponentType.IsAssignableFrom(c.GetType()))
                            .Cast<Object>()
                            .ToArray();
        ...
        {
            if (EditorCommon.BeginFoldout(ChildrenTilesGUI.ID, ChildrenTilesGUI.HEADER, null, true))
            {
        ...
        private class TileSettingsGUI
        {
            ...
            public static readonly GUIContent MISSING_GEOMETRY_ACTION = new GUIContent("Missing Geometry", "What to do when height, hole, or mesh density output is not produced by the current generation.");
            public static readonly GUIContent MISSING_TEXTURE_ACTION = new GUIContent("Missing Textures", "What to do when albedo, metallic, or layer-weight output is not produced by the current generation.");
            public static readonly GUIContent MISSING_POPULATION_ACTION = new GUIContent("Missing Population", "What to do when trees, details, or objects are not produced by the current generation.");

            public static readonly GUIContent TERRAIN_SETUP_HEADER = new GUIContent("Terrain Setup");
            public static readonly GUIContent OUTPUT_HANDLING_HEADER = new GUIContent("Output Handling");

            public static readonly GUIContent EDITION_MESSAGE = new GUIContent("Bring more definition to every tile with higher resolutions in Indie and Pro.");
            public static readonly GUIContent COMPARE_EDITIONS = new GUIContent("Compare editions →");
        }

        private void DrawTileSettingsGUI()
        {
            if (EditorCommon.BeginFoldout(TileSettingsGUI.ID, TileSettingsGUI.HEADER, null, true))
            {
                EditorGUILayout.LabelField(TileSettingsGUI.TERRAIN_SETUP_HEADER, EditorCommon.Styles.h3);
                EditorGUI.BeginChangeCheck();
                float maxHeight = EditorGUILayout.FloatField(TileSettingsGUI.TERRAIN_MAX_HEIGHT, m_instance.terrainMaxHeight);
                int heightmapResolution = EditorCommon.HeightMapResolutionSelector(TileSettingsGUI.HEIGHT_MAP_RESOLUTION, m_instance.heightMapResolution);
                int textureResolution = EditorCommon.TextureResolutionSelector(TileSettingsGUI.TEXTURE_RESOLUTION, m_instance.textureResolution);
                int detailDensityResolution = EditorCommon.TextureResolutionSelector(TileSettingsGUI.DENSITY_MAP_RESOLUTION, m_instance.detailDensityMapResolution);
                ...
                if (EditorCommon.IsPersonalEdition())
                {
                    ...
                    EditorGUILayout.LabelField(TileSettingsGUI.EDITION_MESSAGE, EditorCommon.Styles.infoLabel);
                    if (EditorGUILayout.LinkButton(TileSettingsGUI.COMPARE_EDITIONS))
                    {
                        NetUtils.TrackClick("compare-edition", UILocation.Inspector_VM);
                ...
                EditorGUILayout.LabelField(TileSettingsGUI.OUTPUT_HANDLING_HEADER, EditorCommon.Styles.h3);
                EditorGUI.BeginChangeCheck();
                MissingOutputAction missingGeometryAction = (MissingOutputAction)EditorGUILayout.EnumPopup(TileSettingsGUI.MISSING_GEOMETRY_ACTION, m_instance.missingGeometryAction);
                MissingOutputAction missingTextureAction = (MissingOutputAction)EditorGUILayout.EnumPopup(TileSettingsGUI.MISSING_TEXTURE_ACTION, m_instance.missingTextureAction);
                MissingOutputAction missingPopulationAction = (MissingOutputAction)EditorGUILayout.EnumPopup(TileSettingsGUI.MISSING_POPULATION_ACTION, m_instance.missingPopulationAction);
                bool cullBiomes = EditorGUILayout.Toggle(TileSettingsGUI.CULL_BIOMES, m_instance.shouldCullBiomes);
                if (EditorGUI.EndChangeCheck())
                {
                    Undo.RecordObject(m_instance, $"Modify {m_instance.name}");
                    m_instance.missingGeometryAction = missingGeometryAction;
                    m_instance.missingTextureAction = missingTextureAction;
                    m_instance.missingPopulationAction = missingPopulationAction;
        ...
        private class ActionGUI
        {
            ...
            public static readonly GUIContent FORGET_CURRENT_TASK = new GUIContent("Cancel Generation");
        }
        ...
        private void DrawActionGUI()
        {
            if (EditorCommon.BeginFoldout(ActionGUI.ID, ActionGUI.HEADER, null, true))
            {
                ...
                {
                    GenerationTask task = SceneBiomeService.GenerateAll(m_instance);
                    if (!ReferenceEquals(task, GenerationTask.completedTask))
        ...
        private static void RecordInspectorInteraction()
        {
            SuccessfulActionCounter.Record(
                SuccessfulActionCounter.ActionKeys.VISTA_MANAGER_INSPECTOR_INTERACTION);
        }
