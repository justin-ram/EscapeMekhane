...
namespace Pinwheel.VistaEditor.Graph
{
    public class GraphEditorBase : EditorWindow
    {
        ...
        protected bool m_isDrawingNodeProperties;
        private bool m_hasPendingSuccessfulInteraction;

        internal static GraphAsset currentlyOpeningGraph { get; set; }
        ...
        internal ToolbarButton m_rightPanelButton;
        ...
        public static GraphEditorBase OpenGraph(GraphAsset graph, IExternalInputProvider inputProvider = null)
        {
            GraphEditorUIVersion uiVersion = EditorSettings.Get().graphEditorSettings.uiVersion;
            if (graph is TerrainDataGraph tg)
            {
                switch (uiVersion)
                {
                    case GraphEditorUIVersion.V2:
                        return GraphEditorBase.OpenGraph<TerrainGraphEditor2>(tg, inputProvider);
                    default:
                        return GraphEditorBase.OpenGraph<TerrainGraphEditor>(tg, inputProvider);
                }
            }
            else
            {
        ...
        public static T OpenGraph<T>(GraphAsset graph, IExternalInputProvider inputProvider = null) where T : GraphEditorBase
        {
            ...
            {
                currentlyOpeningGraph = graph;
                T editor = CreateWindow<T>(typeof(GraphEditorBase), typeof(TerrainGraphEditor), typeof(SceneView));
        ...
        public virtual void FocusMainContent()
        {
            if (m_graphView != null)
            {
                m_graphView.Focus();
        ...
        public virtual void OnGUI()
        {
            ...
            TryRecordPendingSuccessfulInteraction();
        }
        ...
        public void OnDestroy()
        {
            ...
            // Defer asset deletion so that a maximize->normal cycle does not destroy the temp asset
            // while the original window is still alive. When Unity maximizes a window it creates a
            // second EditorWindow instance that shares the same m_clonedGraph asset. That copy's
            // OnDestroy fires before the original window's OnEnable re-registers it in openingEditors,
            // so we check one editor tick later, once the surviving window has had a chance to add
            // itself back to the list.
            GraphAsset clonedRef = m_clonedGraph;
            EditorApplication.delayCall += () =>
            {
                bool stillInUse = openingEditors != null && openingEditors.Exists(e => e != null && e.m_clonedGraph == clonedRef);
                if (!stillInUse && AssetDatabase.Contains(clonedRef))
                {
                    string path = AssetDatabase.GetAssetPath(clonedRef);
        ...
        private void OnKeyDown(KeyDownEvent evt)
        {
            if (evt.ctrlKey && evt.keyCode == KeyCode.S)
            {
                ...
                SuccessfulActionCounter.Record(
                    SuccessfulActionCounter.ActionKeys.GRAPH_SAVED);
                evt.StopPropagation();
            }
        }
        ...
        private void RecordSuccessfulInteraction()
        {
            ...
            {
                m_hasPendingSuccessfulInteraction = true;
                return;
            }

            RecordSuccessfulGraphInteraction();
        }

        private void TryRecordPendingSuccessfulInteraction()
        {
            if (m_hasPendingSuccessfulInteraction &&
                GUIUtility.hotControl == 0 &&
                GUIUtility.keyboardControl == 0)
            {
                m_hasPendingSuccessfulInteraction = false;
                RecordSuccessfulGraphInteraction();
            }
        }

        private void RecordSuccessfulGraphInteraction()
        {
            string actionKey = SuccessfulActionCounter.ActionKeys.GRAPH_EDITOR_INTERACTION;
            SuccessfulActionCounter.Record(actionKey);
        }
            ...
            if (activeNode != null)
            {
                ...
                try
                {
                    ...
                    if (nodeEditor.hasSpecificProperties)
                    {
                        ...
                        bool specificPropertiesExpanded = SessionState.GetBool(SPECIFIC_PROPERTIES_FOLDOUT_KEY, false);
        ...
        internal virtual void OnDrawEditorSettings()
        {
        ...
        internal virtual void RequestGraphExecution()
        {
        }
