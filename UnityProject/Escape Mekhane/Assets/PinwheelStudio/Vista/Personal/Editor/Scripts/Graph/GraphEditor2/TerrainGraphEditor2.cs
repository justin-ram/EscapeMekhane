...
{
    public class TerrainGraphEditor2 : TerrainGraphEditor, ICanShowReviewPrompt
    {
        ...
        private bool m_isNodePropertiesPositionUserDefined;
        private bool m_hasAppliedViewport2DDefaultGeometry;
        private bool m_hasAppliedViewport3DDefaultGeometry;
        private bool m_isApplyingViewport2DDefaultGeometry;
        private bool m_isApplyingViewport3DDefaultGeometry;
        ...
        protected override void OnSetupGUI()
        {
            ...
            ApplyViewportDefaultGeometryOnFirstShow(m_viewport2DSubWindow, false);
            ...
            ApplyViewportDefaultGeometryOnFirstShow(m_viewport3DSubWindow, true);
            ...
            m_nodePropertiesSubWindow.positionDragged += OnNodePropertiesPositionDragged;
            m_bodyContainer.Add(m_nodePropertiesSubWindow);
            m_nodePropertiesSubWindow.SetSize(new Vector2(360, 300));
        ...
        private void ApplyViewportDefaultGeometryOnFirstShow(MovableSubWindow viewport, bool is3D)
        {
            bool hasApplied = is3D ? m_hasAppliedViewport3DDefaultGeometry : m_hasAppliedViewport2DDefaultGeometry;
            bool isApplying = is3D ? m_isApplyingViewport3DDefaultGeometry : m_isApplyingViewport2DDefaultGeometry;
            if (hasApplied || isApplying || !SubWindowManager.IsVisibleBySettings(viewport.viewDataKey))
                return;

            if (TryApplyViewportDefaultGeometry(viewport, is3D))
                return;

            if (is3D)
            {
                m_isApplyingViewport3DDefaultGeometry = true;
            }
            else
            {
                m_isApplyingViewport2DDefaultGeometry = true;
            }
            CoroutineUtility.StartCoroutine(IApplyViewportDefaultGeometryWhenReady(viewport, is3D));
        }

        private IEnumerator IApplyViewportDefaultGeometryWhenReady(MovableSubWindow viewport, bool is3D)
        {
            while (SubWindowManager.IsVisibleBySettings(viewport.viewDataKey) &&
                !TryApplyViewportDefaultGeometry(viewport, is3D))
            {
                yield return null;
            }
            ...
            if (is3D)
            {
                m_isApplyingViewport3DDefaultGeometry = false;
            }
            else
            {
                m_isApplyingViewport2DDefaultGeometry = false;
            }
        }

        private bool TryApplyViewportDefaultGeometry(MovableSubWindow viewport, bool is3D)
        {
            float graphViewHeight = m_graphView.layout.height;
            if (graphViewHeight <= 0 || float.IsNaN(graphViewHeight))
                return false;

            float viewportSize = graphViewHeight * 0.5f;
            viewport.SetSize(new Vector2(viewportSize, viewportSize));
            viewport.SetPosition(new Vector2(0, is3D ? 0 : viewportSize));

            if (is3D)
            {
                m_hasAppliedViewport3DDefaultGeometry = true;
            }
            else
            {
                m_hasAppliedViewport2DDefaultGeometry = true;
            }
            return true;
        }
        ...
        protected IEnumerator IOnActiveNodeChangedDelayed(NodeView nv)
        {
            ...
            else
            {
                SubWindowManager.Hide(m_nodePropertiesSubWindow);
                SubWindowManager.Show(m_nodePropertiesSubWindow);
                if (!m_isNodePropertiesPositionUserDefined)
                {
        ...
        private void OnNodePropertiesPositionDragged()
        {
            m_isNodePropertiesPositionUserDefined = true;
        }
        ...
        internal override void OnDrawNodeProperties()
        {
            ...
            if (activeNode != null)
            {
                ...
                m_isDrawingNodeProperties = true;
                try
                {
                ...
                {
                    m_isDrawingNodeProperties = false;
                }
            }
            else
            {
        ...
        protected override void UpdateToolbarButtonsStatus()
        {
            ...
            m_splitExecButton.SetToggled((clonedGraph as TerrainDataGraph).allowSplitExecution);
