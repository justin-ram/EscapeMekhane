...
namespace Pinwheel.VistaEditor.Graph
{
    ...
    public class InputNodeEditor : ExecutableNodeEditorBase, INeedUpdateNodeVisual
    {
        ...
        private static readonly GUIContent FALLBACK = new GUIContent(
            "Fallback",
            "Texture used when the graph execution does not provide this external input.");
        ...
        public void UpdateVisual(INode node, NodeView nv)
        {
            ...
            if (!string.IsNullOrEmpty(n.fallbackWarning))
            {
                nv.AddWarning(n.fallbackWarning);
            }
        }
        ...
        public override void OnGUI(INode node)
        {
            ...
            if (nameSelector.Count > 0)
            {
                ...
                if (GUI.Button(dropDownRect, "", EditorStyles.popup))
                {
                    ...
                    {
                        menu.AddItem(
                            new GUIContent(!string.IsNullOrEmpty(s.displayName) ? s.displayName : s.name),
                            false,
                            () =>
                            {
            ...
            Texture fallbackTexture = n.fallbackTexture;
            if (n.slotType != typeof(BufferSlot))
            {
                fallbackTexture = EditorGUILayout.ObjectField(FALLBACK, fallbackTexture, typeof(Texture), false) as Texture;
            }
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                n.fallbackTexture = fallbackTexture;
                m_graphEditor.UpdateNodesVisual();
            }
        }
    }
