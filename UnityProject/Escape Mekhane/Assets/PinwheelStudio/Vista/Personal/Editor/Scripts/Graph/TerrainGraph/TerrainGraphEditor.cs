...
namespace Pinwheel.VistaEditor.Graph
{
    public partial class TerrainGraphEditor : GraphEditorBase
    {
        ...
        internal HelpBox m_templateUsageWarning;
        ...
        private CancellationHandle m_lastExecutionCancellation;
        private bool m_hasPendingExecution;
        private bool m_shouldCancelExecution;
        private bool m_externalInputNeedsCleanup;
        private float m_lastLivePreviewProgress = -1f; //tracks bake advancement so live-preview viewport refresh only fires when progress moves
        internal GraphExecutionCache m_cache;
        ...
        public void Awake()
        {
            ...
            m_lastExecutionCancellation = default;
            m_hasPendingExecution = false;
            m_shouldCancelExecution = false;
            m_externalInputNeedsCleanup = false;
        }
        ...
        public override void OnDisable()
        {
            base.OnDisable();
            SavePrefs();
            DisposeLastExecution();
            CleanUpExternalInput();
            DisposeCache();
            //CleanUpEnvironment();
        }

        private GraphExecutionCache EnsureCache()
        {
            if (m_cache == null)
            {
                m_cache = new GraphExecutionCache();
            }
            return m_cache;
        }
        ...
        private void DisposeLastExecution()
        {
            m_hasPendingExecution = false;
            m_shouldCancelExecution = false;
            if (m_lastExecution != null)
            {
                m_lastExecutionCancellation?.Cancel();
                m_lastExecution.Dispose();
                m_lastExecution = null;
            }
            m_lastExecutionCancellation = null;
        }
        ...
        protected virtual void OnViewportKeyDown(KeyDownEvent evt)
        {
            if (evt.keyCode == KeyCode.Alpha2 && !evt.ctrlKey && !evt.altKey && !evt.shiftKey)
            {
                Toggle2DViewport();
                evt.StopPropagation();
            }
            else if (evt.keyCode == KeyCode.Alpha3 && !evt.ctrlKey && !evt.altKey && !evt.shiftKey)
            {
                Toggle3DViewport();
                evt.StopPropagation();
            }
        ...
        protected virtual void Toggle2DViewport()
        {
            OnDebug2dButtonClicked();
        }

        protected virtual void Toggle3DViewport()
        {
            OnDebug3dButtonClicked();
        }

        protected override void OnSetupGUI()
        {
            m_templateUsageWarning = new HelpBox(string.Empty, HelpBoxMessageType.Warning) { name = "template-usage-warning" };
            m_templateUsageWarning.style.display = new StyleEnum<DisplayStyle>(DisplayStyle.None);
            m_templateUsageWarning.style.marginLeft = new StyleLength(4f);
            m_templateUsageWarning.style.marginTop = new StyleLength(4f);
            m_templateUsageWarning.style.marginRight = new StyleLength(4f);
            m_templateUsageWarning.style.marginBottom = new StyleLength(4f);
            rootVisualElement.Insert(1, m_templateUsageWarning);
        ...
        protected override void OnRegisterCallbacks()
        {
            ...
            TerrainDataGraph.onAfterGraphExecution += OnAfterGraphExecution;
            TerrainDataGraph.onGraphExecutionEnded += OnGraphExecutionEnded;
            EditorApplication.projectChanged += OnProjectChanged;
            rootVisualElement.RegisterCallback<KeyDownEvent>(OnViewportKeyDown);
        }
        ...
        protected override void OnUnregisterCallbacks()
        {
            ...
            TerrainDataGraph.onGraphExecutionEnded -= OnGraphExecutionEnded;
            EditorApplication.projectChanged -= OnProjectChanged;
            rootVisualElement.UnregisterCallback<KeyDownEvent>(OnViewportKeyDown);
        }
        ...
        private void RefreshTemplateUsageWarning()
        {
            if (m_templateUsageWarning == null)
            {
                return;
            }

            TerrainGraph terrainGraph = sourceGraph as TerrainGraph;
            if (terrainGraph != null && BiomeTemplateUtils.TryGetTemplateUsingGraph(terrainGraph, out BiomeTemplate template))
            {
                string templateName = !string.IsNullOrEmpty(template.info.title) ? template.info.title : template.name;
                m_templateUsageWarning.text = $"This graph is used by biome template \"{templateName}\". Editing it here will change the source graph used for future biome creation.";
                m_templateUsageWarning.style.display = new StyleEnum<DisplayStyle>(DisplayStyle.Flex);
            }
            else
            {
                m_templateUsageWarning.text = string.Empty;
                m_templateUsageWarning.style.display = new StyleEnum<DisplayStyle>(DisplayStyle.None);
            }
        }
        ...
        internal override void RequestGraphExecution()
        {
            EditorSettings editorSettings = EditorSettings.Get();
            if (editorSettings.troubleshootingSettings.dontExecuteGraphOnSelection)
            {
                //Discard any queued rebake and record that active work should stop.
                m_hasPendingExecution = false;
                m_shouldCancelExecution = true;
            }
            else
            {
                //There is only one pending slot. Repeated edits coalesce, and the eventual execution
                //reads the newest graph/editor state instead of replaying stale requests.
                m_hasPendingExecution = true;
                m_shouldCancelExecution = true;
            }
        }

        private void CoordinateGraphExecution()
        {
            //Phase 1: settle the previous execution before considering a replacement.
            if (m_lastExecution != null)
            {
                if (!m_lastExecution.isCompleted)
                {
                    //Cancellation is a one-shot command. The CancellationSignal retains the requested
                    //state while the graph cooperatively unwinds over subsequent editor updates.
                    if (m_shouldCancelExecution)
                    {
                        m_lastExecutionCancellation?.Cancel();
                        m_shouldCancelExecution = false;
                    }
                }
                else if (m_hasPendingExecution || m_lastExecution.isCancellationRequested)
                {
                    //A completed run is discarded when it was cancelled or a newer preview must replace it.
                    m_shouldCancelExecution = false;
                    DisposeCompletedExecution();
                }
                else
                {
                    //Keep a successful completed execution alive because its DataPool backs the preview.
                    m_shouldCancelExecution = false;
                }
            }
            else
            {
                //There is no active execution to receive a cancellation command.
                m_shouldCancelExecution = false;
            }

            //Phase 2: start the newest pending request only after phase 1 has left no old execution.
            if (m_lastExecution == null && m_hasPendingExecution)
            {
                StartPendingExecution();
            }
        }

        private void StartPendingExecution()
        {
            if (!m_hasPendingExecution)
                return;
            ...
            if (ids.Count > 0)
            {
                ...
                TerrainDataGraph.FillArgumentsHandler fillArgumentsCallback = null;
                if (m_externalInputProvider != null)
                {
                    m_externalInputNeedsCleanup = true;
                    try
                    {
                        if (m_externalInputProvider is TerrainSubGraphInputProvider subGraphInputProvider)
                        {
                            if (!subGraphInputProvider.TrySetInput(inputContainer, clonedGraph))
                            {
                                m_externalInputNeedsCleanup = false;
                                m_hasPendingExecution = true;
                                return;
                        ...
                        {
                            m_externalInputProvider.SetInput(inputContainer, clonedGraph);
                        }
                ...
                TerrainDataGraph tg = clonedGraph as TerrainDataGraph;
                if (m_externalInputProvider != null)
                {
                    tg.debugConfigs = m_externalInputProvider.GetDebugConfigs();
                }
                m_lastExecutionCancellation = new CancellationHandle();
                m_lastExecution = tg.Execute(ids.ToArray(), tg.debugConfigs, inputContainer, fillArgumentsCallback, EnsureCache(), m_lastExecutionCancellation.signal);
            }
        }

        private void DisposeCompletedExecution()
        {
            if (m_lastExecution == null)
                return;
            if (!m_lastExecution.isCompleted)
        ...
        internal bool TryExecuteGraphImmediateAt(out DataPool data, params string[] nodeIds)
        {
            data = null;
            m_hasPendingExecution = false;
            if (m_lastExecution != null && !m_lastExecution.isCompleted)
            {
                m_shouldCancelExecution = true;
                return false;
            }
            ...
            if (nodeIds.Length > 0)
            {
                ...
                data = tg.ExecuteImmediate(nodeIds, tg.debugConfigs, inputContainer, fillArgumentsCallback, EnsureCache());
            }
            return true;
        }
        ...
        private void OnGraphExecutionEnded(TerrainDataGraph graph, TerrainGenerationConfigs data, string[] ids, bool isCancelled)
        {
            if (graph != clonedGraph)
                return;
        ...
        private void CleanUpExternalInput()
        {
            if (m_externalInputProvider != null && m_externalInputNeedsCleanup)
            {
                m_externalInputNeedsCleanup = false;
        ...
        {
            string text =
                $"Texture: {m_currentMemStats.textureCount}/{m_peakMemStats.textureCount} ׀ " +
                $"Buffer: {m_currentMemStats.bufferCount}/{m_peakMemStats.bufferCount} ׀ " +
                $"Memory: {m_currentMemStats.megabyte.ToString("0.0")}/{m_peakMemStats.megabyte.ToString("0.0")} MB ׀ " +
                $"Version: {VersionInfo.versionLabel}";
            m_statusBar.infoRight = text;
        }

        public void Update()
        {
            CoordinateGraphExecution();
            ...
            if (m_lastExecution != null)
            {
                if (m_lastExecution.progress != null)
                {
                    if (m_statusBar.IsProgressVisible())
                    {
                        ...
                        //Live preview: while a bake runs, refresh the viewports as it advances so nodes that write their
                        //output progressively (e.g. HydraulicErosionNode2) show the terrain evolving instead of only at the end.
                        //Each viewport is gated on its display node's output already existing in the current pool, so a
                        //re-bake of a non-progressive node keeps showing the previous result instead of flashing empty.
                        float progressSignal = m_lastExecution.progress.totalProgress + m_lastExecution.progress.currentProgress;
                        if (!Mathf.Approximately(progressSignal, m_lastLivePreviewProgress))
                        {
                            m_lastLivePreviewProgress = progressSignal;
                            if (m_viewport3d != null && m_viewport3d.enabledInHierarchy && m_viewport3d.HasDisplayableHeight())
                            {
                                m_viewport3d.RenderViewport();
                                m_viewport3d.MarkDirtyRepaint();
                            }
                            if (m_viewport2d != null && m_viewport2d.enabledInHierarchy && m_viewport2d.HasDisplayableContent())
                            {
                                m_viewport2d.RenderViewport();
                                m_viewport2d.MarkDirtyRepaint();
                            }
        ...
        private void UpdateTroubleshootingStatus()
        {
            ...
            {
                string[] logs = EditorSettings.TroubleshootingSettings.GetGraphExecLog(m_clonedGraph as TerrainDataGraph);
                m_graphView.Query<NodeView>().ForEach((nv) =>
                {
        ...
        protected void OnDebug3dButtonClicked()
        {
            if (m_viewport3d != null)
            {
        ...
        protected void OnDebug2dButtonClicked()
        {
            if (m_viewport2d != null)
            {
        ...
        protected virtual void UpdateToolbarButtonsStatus()
        {
        ...
        protected virtual bool IsViewport3DVisible()
        {
            ...
            return isViewport3dVisible;
        }

        protected virtual bool IsViewport2DVisible()
        {
        ...
        private void UpdateNodeVisual(NodeView nv)
        {
            ...
            {
                nv.SetBadgeEnable<SplitExecutionBadge>(enb.shouldSplitExecution && (clonedGraph as TerrainDataGraph).allowSplitExecution);
