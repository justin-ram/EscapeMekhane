...
namespace Pinwheel.VistaEditor.Graph
{
    public class TerrainGraphViewport2d : Viewport
    {
        ...
        private RenderTexture m_visualizationRt;
        private RenderTexture m_graphOutputRt;
        ...
        public void RenderViewport()
        {
            ...
            {
                m_graphOutputRt = null;
                image = Texture2D.blackTexture;
                m_textureInfoText.text = "";
            }
            else
            {
                m_graphOutputRt = info.renderTexture;
                ...
                else
                {
                    ...
                    if (m_visualizationRt == null)
                    {
                        int rtWidth = Mathf.Max(1, info.renderTexture.width);
                        int rtHeight = Mathf.Max(1, info.renderTexture.width);
                        m_visualizationRt = new RenderTexture(rtWidth, rtHeight, 16, RenderTextureFormat.ARGB32);
                    }
                    if (m_material == null)
                    {
                    ...
                    Drawing.DrawQuad(m_visualizationRt, m_material, 0);

                    image = m_visualizationRt;
                    m_textureInfoText.text = info.infoText;
                }
            }
        }
        ...
        private void CleanUp()
        {
            ...
            if (m_visualizationRt != null)
            {
                m_visualizationRt.Release();
                m_visualizationRt = null;
            }
            if (m_material != null)
            {
        ...
        //True when the current (possibly in-progress) execution already has an RT for the display node's first output
        //slot. Gates the live-preview refresh so we only re-render when there is real data, keeping the previous frame
        //on screen during a non-progressive node's bake instead of flashing empty.
        internal bool HasDisplayableContent()
        {
            if (m_editor.m_lastExecution == null || m_editor.clonedGraph == null)
                return false;
            INode n = m_editor.clonedGraph.GetNode(m_editor.m_display2dNodeId);
            if (n == null)
            {
            ...
            return m_editor.m_lastExecution.data.GetRT(textureName) != null;
        }
        ...
        {
            if (m_graphOutputRt != null && m_getPixelShader != null)
            {
                m_pixelInfoText.style.display = new StyleEnum<DisplayStyle>(DisplayStyle.Flex);
                Vector2 pixelPos = new Vector2(
                    Mathf.FloorToInt(uv.x * (m_graphOutputRt.width - 1)),
                    Mathf.FloorToInt(uv.y * (m_graphOutputRt.height - 1)));
                ...
                m_getPixelShader.SetTexture(GetPixelConfig.KERNEL, GetPixelConfig.TEXTURE, m_graphOutputRt);
