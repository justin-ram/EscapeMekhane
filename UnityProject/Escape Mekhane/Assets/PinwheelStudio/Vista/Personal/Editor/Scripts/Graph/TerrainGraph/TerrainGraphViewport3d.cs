...
namespace Pinwheel.VistaEditor.Graph
{
    public class TerrainGraphViewport3d : Viewport
    {
        ...
        private RenderTexture m_normalMapOS;
        ...
        private static readonly int NORMAL_MAP = Shader.PropertyToID("_NormalMapOS");
        ...
        public TerrainGraphViewport3d(TerrainGraphEditor editor) : base()
        {
            ...
            m_toggleWaterButton.tooltip = "Toggle water rendering. You can change the water level in graph editor settings.";
        ...
        private void CleanUp()
        {
            ...
            if (m_normalMapOS != null)
            {
                m_normalMapOS.Release();
                m_normalMapOS = null;
            }
            if (m_terrainMaterial != null)
            {
        ...
        private void RenderTerrain(Camera camera, int gridSize)
        {
            ...
            ExtractNormalMap(setup.heightMap);
            m_terrainMaterial.SetTexture(NORMAL_MAP, m_normalMapOS);
            ...
            TerrainDataGraph graph = m_editor.clonedGraph as TerrainDataGraph;
            ...
            for (int x = start; x <= end; ++x)
            {
                for (int z = start; z <= end; ++z)
                {
                    ...
                    Utilities.DrawMeshCompat(m_patchMesh, trs, m_terrainMaterial, 0, camera, 0, props, ShadowCastingMode.Off, false, null, LightProbeUsage.Off);
                }
            }
        ...
        private void ExtractNormalMap(RenderTexture heightMap)
        {
            if (heightMap != null)
            {
                if (m_normalMapOS != null && (m_normalMapOS.width != heightMap.width || m_normalMapOS.height != heightMap.height))
                {
                    m_normalMapOS.Release();
                    m_normalMapOS = null;
                }
                if (m_normalMapOS == null)
                {
                    m_normalMapOS = new RenderTexture(heightMap.width, heightMap.height, 0, RenderTextureFormat.ARGB32, RenderTextureReadWrite.Linear);
                    m_normalMapOS.enableRandomWrite = true;
                    m_normalMapOS.wrapMode = TextureWrapMode.Clamp;
                    m_normalMapOS.filterMode = FilterMode.Bilinear;
                }

                TerrainDataGraph graph = m_editor.clonedGraph as TerrainDataGraph;
                Vector3 terrainSize = new Vector3(graph.debugConfigs.worldBounds.width, graph.debugConfigs.terrainHeight, graph.debugConfigs.worldBounds.height);
                VistaLib.ExtractNormalMapOS(m_normalMapOS, heightMap, terrainSize);
            }
            else
            {
                if (m_normalMapOS == null)
                {
                    m_normalMapOS = new RenderTexture(8, 8, 0, RenderTextureFormat.ARGB32, RenderTextureReadWrite.Linear);
                    m_normalMapOS.enableRandomWrite = true;
                    m_normalMapOS.wrapMode = TextureWrapMode.Clamp;
                    m_normalMapOS.filterMode = FilterMode.Bilinear;
                }
                //Up vector mapped from [-1,1] to [0, 1]
                VistaLib.FillColor(m_normalMapOS, new Color(0.5f, 1f, 0.5f, 0));
            }
        }
        ...
        private void RenderWater(Camera camera)
        {
            ...
            Utilities.DrawMeshCompat(m_patchMesh, trs, m_waterMaterial, 0, camera, 0, null, ShadowCastingMode.Off, false, null, LightProbeUsage.Off);
        }
        ...
        //True when the current (possibly in-progress) execution already has a height RT for the display node's
        //first output slot. Used to gate the live-preview refresh so we only re-render when there is real data to
        //show, keeping the previous frame on screen during a non-progressive node's bake instead of flashing empty.
        internal bool HasDisplayableHeight()
        {
            if (m_editor.m_lastExecution == null || m_editor.clonedGraph == null)
                return false;
            EditorSettings editorSettings = EditorSettings.Get();
            ...
            return m_editor.m_lastExecution.data.GetRT(outputRef) != null;
        }
        ...
        private void OnTakeScreenshotButtonClicked()
        {
            ...
            string baseDirectory = settings.graphEditorSettings.fileExportDirectory;
            string folder = Path.Combine(baseDirectory, "Screenshots/");
