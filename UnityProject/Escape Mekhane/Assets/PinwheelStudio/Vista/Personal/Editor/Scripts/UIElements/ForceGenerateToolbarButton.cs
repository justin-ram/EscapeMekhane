...
namespace Pinwheel.VistaEditor.UIElements
{
    ...
    public class ForceGenerateToolbarButton : EditorToolbarButton
    {
        ...
        {
            if (VistaManager.allInstances.Any(vm => vm.isGenerating))
            {
        ...
        {
            VistaManager[] vms = Utilities.FindObjectsNoSortCompat<VistaManager>();

            IReadOnlyList<GenerationTask> tasks = SceneBiomeService.GenerateAll(vms);
