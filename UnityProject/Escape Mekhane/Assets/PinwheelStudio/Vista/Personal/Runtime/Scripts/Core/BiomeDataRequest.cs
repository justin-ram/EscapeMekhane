...
namespace Pinwheel.Vista
{
    ...
    public class BiomeDataRequest : ProgressiveTask
    {
        ...
        /// <summary>Gets the exception that prevented this request from producing a valid result.</summary>
        public Exception exception { get; private set; }

        /// <summary>Gets whether this request terminated because its producer failed.</summary>
        public bool isFaulted => exception != null;

        internal void Fail(Exception failure)
        {
            exception = failure ?? new InvalidOperationException("Biome data production failed without an exception.");
            Complete();
        }
    }
}
