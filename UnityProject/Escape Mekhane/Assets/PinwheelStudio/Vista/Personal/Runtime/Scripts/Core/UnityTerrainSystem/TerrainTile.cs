...
{
    /// <summary>
    /// Connects a Unity <see cref="Terrain"/> to Vista so it can be generated, updated, and protected by Terrain Seals.
    /// </summary>
    /// <remarks>
    /// Add this component beside a Unity Terrain and assign it to a <see cref="VistaManager"/> through
    /// <see cref="managerId"/>. During generation, Vista can replace the terrain's height, holes, painted layers, trees,
    /// details, and spawned objects. The tile also lets Terrain Seals keep supported manual terrain edits through later
    /// regeneration and exposes generic outputs to custom processors through its callback events.
    ///
    ...
    public class TerrainTile : MonoBehaviour, ITile, ITileSnapshotProvider, IGeometryPopulator, ILayerWeightsPopulator, ITreePopulator, IDetailDensityPopulator, IObjectPopulator, IGenericTexturePopulator, IGenericBufferPopulator, ISceneHeightProvider, ISceneTextureWeightProvider
    {
        /// <summary>
        ...
        /// <summary>
        /// Gets the Unity Terrain component that receives Vista's generated data.
        /// </summary>
        ...
        /// </summary>
        /// <remarks>
        /// The bounds are derived from the terrain transform and <see cref="TerrainData.size"/>.
        /// </remarks>
        ...
        /// <summary>
        /// Gets or sets the world-height range represented by normalized terrain heights.
        /// </summary>
        /// <remarks>
        ...
        /// <summary>
        /// Gets or sets the height-map resolution used for Vista generation and Unity Terrain storage.
        /// </summary>
        /// <remarks>
        ...
        /// <summary>
        /// Gets or sets the resolution used for generated terrain-layer weights and Unity alphamaps.
        /// </summary>
        /// <remarks>
        ...
        /// <summary>
        /// Gets or sets the resolution used for generated grass and detail density.
        /// </summary>
        /// <remarks>
        ...
        /// <summary>
        /// Gets the consolidated terrain-layer array most recently assigned to Unity Terrain by Vista.
        /// </summary>
        /// <remarks>
        ...
        /// <summary>
        /// Starts a generation-scoped capture of the current Unity Terrain data for Terrain Seal preservation.
        /// </summary>
        /// <remarks>
        /// The snapshot contains the current height, holes, terrain-layer weights, trees, and detail density in Vista's
        /// standard <see cref="BiomeData"/> representation. Capture creates fresh template adapters for live Unity
        /// prototypes and registers them with <paramref name="transientResources"/>. The request's caller owns the returned
        /// <see cref="BiomeData"/> and must dispose it after the tile pass.
        /// </remarks>
            ...
            CancellationSignal cancellationSignal = null)
        {
            ...
            if (terrain == null || terrain.terrainData == null)
            {
                throw new System.InvalidOperationException("Cannot capture a Unity Terrain snapshot without TerrainData.");
            }
        ...
        {
            return TerrainTileUtilities.IsEquivalent(first, second);
        }
        ...
        {
            TerrainTileUtilities.CaptureHeightMap(request.data, terrainData);
            yield return null;
            ...
            TerrainTileUtilities.CaptureHoleMap(request.data, terrainData);
            if (cancellationSignal?.isCancellationRequested == true)
            {
                request.Complete();
                yield break;
            ...
            TerrainTileUtilities.CaptureLayerWeights(request.data, terrainData);
            if (cancellationSignal?.isCancellationRequested == true)
            {
                request.Complete();
                yield break;
            ...
            yield return TerrainTileUtilities.CaptureTreesProgressive(
            ...
            yield return TerrainTileUtilities.CaptureDetailDensityProgressive(
                request.data, terrainData, transientResources, cancellationSignal);
            if (cancellationSignal?.isCancellationRequested == true)
            {
                request.Complete();
            ...
            TerrainTileUtilities.CaptureSnapshotResults(request.data, terrainData);
            request.Complete();
            yield break;
        }
        ...
        {
            const float holeThreshold = 0.5f;
            ...
            GraphicsUtils.ReadRenderTexture(scaledHoleMap, map, false);

            NativeArray<float> data = map.GetRawTextureData<float>();
            const int rowsPerBand = 8;
            float[] dataBand = new float[resolution * Mathf.Min(rowsPerBand, resolution)];
            bool[,] holes = new bool[resolution, resolution];
            for (int baseY = 0; baseY < resolution; baseY += rowsPerBand)
            {
                int bandHeight = Mathf.Min(rowsPerBand, resolution - baseY);
                int bandLength = bandHeight * resolution;
                NativeArray<float>.Copy(data, baseY * resolution, dataBand, 0, bandLength);
                for (int y = 0; y < bandHeight; ++y)
                {
                    int rowStart = y * resolution;
                    for (int x = 0; x < resolution; ++x)
                    {
                        holes[baseY + y, x] = dataBand[rowStart + x] < holeThreshold;
                    }
                }
            }
        ...
        /// <summary>
        /// Replaces this tile's Unity Terrain tree prototypes and instances with Vista's generated tree output.
        /// </summary>
        ...
        /// <remarks>
        /// Repeated template references share one Unity prototype. Every valid instance uses that template's primary prefab.
        /// </remarks>
        public void PopulateTrees(List<TreeTemplate> templates, List<ComputeBuffer> buffers)
        {
            // Build one Unity prototype for each unique Vista tree template.
            // Each unique template contributes only its primary prefab.
            ...
            for (int i = 0; i < distinctTemplates.Count; ++i)
            {
                ...
                TreePrototype prototype = CreateTreePrototypeFromTemplate(template);
                protoIndices[i] = prototype != null ? prototypes.Count : -1;
                if (prototype != null)
                    prototypes.Add(prototype);
            }

            // Convert every source buffer into Unity tree instances that point to its template's prototype.
        ...
        private TreePrototype CreateTreePrototypeFromTemplate(TreeTemplate template)
        {
            if (!template.IsValid())
                return null;

            return TerrainTileUtilities.CreateTreePrototype(template);
        }

        private void ParseTreeInstances(List<TreeInstance> instances, ComputeBuffer buffer, int prototypeIndex)
        {
            ...
            // Invalid samples are skipped; every valid sample uses this template's one prototype.
        ...
        /// <remarks>
        /// Duplicated templates are merged first, then each distinct template creates one <see cref="DetailPrototype"/>
        /// from its primary prefab or texture. Density is written patch-by-patch over multiple frames.
        /// </remarks>
        ...
        /// </summary>
        /// <returns>A progressive task that completes after all detail layers and prototypes have been removed.</returns>
        ...
        private IEnumerator PopulateDetailDensityProgressive(ProgressiveTask task, List<DetailTemplate> templates, List<RenderTexture> densityMaps)
        {
            ...
            for (int i = 0; i < distinctTemplates.Count; ++i)
            {
                ...
                DetailPrototype prototype = CreateDetailPrototypeFromTemplate(template);
                if (prototype != null)
                {
                    prototypes.Add(prototype);
                    densityMapByProto.Add(preparedDensityMaps[i]);
                    baseDensityByProto.Add(template.density);
                }
            }
            ...
            out List<Texture2D> cpuDensityMaps)
        {
            ...
            for (int i = 0; i < distinctTemplates.Count; ++i)
            {
                ...
                if (!bucket.needsMergeOrRescale)
                {
                    ...
                    GraphicsUtils.ReadRenderTexture(densityMap, cpuDensityMap, false);
                ...
                else
                {
                    ...
                    Texture2D cpuDensityMap = new Texture2D(resolution, resolution, TextureFormat.RFloat, false);
                    GraphicsUtils.ReadRenderTexture(mergedDensityMap, cpuDensityMap, false);
                    cpuDensityMaps.Add(cpuDensityMap);
        ...
        private IEnumerator PopulateLayerDensityProgressive(int layerIndex, Texture2D densityMap, float density)
        {
            ...
            // The density values are read as raw floats below, which only lines up with a single-channel
            // float layout. PrepareDetailDensityMapsForCpuRead always produces that, so a mismatch means a
            // caller changed the format and would otherwise silently scatter garbage density.
            if (densityMap.format != TextureFormat.RFloat)
            {
                Debug.LogError($"Detail density map must be {TextureFormat.RFloat} to be read as raw floats, but it is {densityMap.format}.");
                yield break;
            }
            ...
            // Allocated once and reused by every step, replacing the Color[] that GetPixels used to
            // allocate per step at four times the width of the underlying floats.
            float[] densityBand = new float[resolutionPerPatch * resolution];
            for (int i = 0; i < stepCount; ++i)
            {
                int baseX = 0;
                int baseY = i * resolutionPerPatch;
                ...
                // A zero-copy view over the texture's existing CPU data. It is re-acquired each step
                // instead of held across the yield, so the view is never used after a frame boundary,
                // and it is never disposed because it does not own the memory.
                NativeArray<float> data = densityMap.GetPixelData<float>(0);
                // Copy the band once so the hot loop uses managed-array indexing instead of paying a
                // NativeArray bounds and safety-handle check for every density sample.
                NativeArray<float>.Copy(data, baseY * resolution, densityBand, 0, densityBand.Length);
                FillDensityArray(densityArray, blockWidth, blockHeight, densityBand, density, layerIndex);
        ...
        private DetailPrototype CreateDetailPrototypeFromTemplate(DetailTemplate t)
        {
            if (!t.IsValid())
                return null;

            return TerrainTileUtilities.CreateDetailPrototype(t);
        }

        /// <summary>
        /// Converts one horizontal band of a density map into the integer density array Unity Terrain expects.
        /// </summary>
        /// <param name="data">
        /// Raw single-channel float pixels for the band being filled, laid out as <paramref name="height"/>
        /// rows of <paramref name="width"/> values.
        /// </param>
        private void FillDensityArray(int[,] array, int width, int height, float[] data, float baseDensity, int randomSeed)
        {
            ...
            for (int y = 0; y < height; ++y)
            {
                ...
                {
                    float densityFloat = data[rowStart + x] * baseDensity;
        ...
        /// <remarks>
        /// Existing spawned-object hierarchy under the terrain is cleared first. Each valid sample uses the primary
        /// prefab, converts normalized sample coordinates into terrain-local placement, applies sampled
        ...
        /// </summary>
        /// <returns>The material then the terrain data, or nothing when the tile has none.</returns>
        public IEnumerable<UnityEngine.Object> GetDataAssets()
        {
            ...
            // The material is yielded before the terrain data, so a caller that saves them creates the material as
            // its own asset first instead of embedding it.
            if (terrain.materialTemplate != null)
            {
                yield return terrain.materialTemplate;
            }
            if (terrain.terrainData != null)
            {
        ...
        public void OnCollectSceneTextureWeight(TerrainLayer terrainLayer, RenderTexture targetRt, Rect requestedWorldRect)
        {
            ...
            int layerIndex = System.Array.IndexOf(terrainData.terrainLayers, terrainLayer);
            if (layerIndex < 0)
                return;
            ...
            combiner.ExtractChannel(
                terrainData.GetAlphamapTexture(layerIndex / 4),
                layerIndex % 4,
                targetRt,
                uvCorner);
        }
