...
namespace Pinwheel.Vista.Graph
{
    ...
    {
        /// <summary>
        /// Processes a polygonal mask through a biome mask graph and returns the graph-produced mask.
        /// </summary>
        /// <param name="graph">Biome mask graph to execute.</param>
        /// <param name="worldBounds">World-space bounds represented by the polygonal mask.</param>
        /// <param name="simSpace">Coordinate space exposed to the graph.</param>
        /// <param name="polygonalMask">Caller-owned mask supplied as the biome-mask graph input.</param>
        /// <returns>
        /// The biome-mask output texture removed from the graph data pool, or null when the graph has no biome-mask output.
        /// The caller owns the returned texture.
        /// </returns>
        public static RenderTexture ProcessPolygonalMask(
            BiomeMaskGraph graph,
            Bounds worldBounds,
            Space simSpace,
            RenderTexture polygonalMask)
        {
            ...
            configs.resolution = polygonalMask.width;
            configs.seed = 0;
            configs.terrainHeight = worldBounds.size.y;
            configs.worldBounds = new Rect(
                simSpace == Space.World ? worldBounds.min.x : 0,
                simSpace == Space.World ? worldBounds.min.z : 0,
                worldBounds.size.x,
                worldBounds.size.z);

            GraphInputContainer inputContainer = new GraphInputContainer();
            inputContainer.AddTexture(GraphConstants.BIOME_MASK_INPUT_NAME, polygonalMask);

            DataPool data = graph.ExecuteImmediate(new string[] { targetOutputNode.id }, configs, inputContainer);
            RenderTexture processedMask = data.RemoveRTFromPool(
                new SlotRef(targetOutputNode.id, targetOutputNode.mainOutputSlot.slotId));
            data.Dispose();
