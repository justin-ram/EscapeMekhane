    ...
    public abstract partial class GraphAsset : ScriptableObject, ISerializationCallbackReceiver
    {
        ...
        public virtual void OnAfterDeserialize()
        {
            using (Serializer.TargetScope s = new Serializer.TargetScope(this)) 
            {
                if (m_nodeData != null)
                {
                    m_nodes = Serializer.Deserialize<INode>(m_nodeData, CreateMissingNodeFallback);
                }
                else
                {
                    m_nodes = new List<INode>();
                ...
                SynthesizeMissingNodeSlotsFromEdges();
        ...
        private static INode CreateMissingNodeFallback(Serializer.JsonObject item)
        {
            MissingNode node = new MissingNode();
            node.SetPreservedSerializedData(item);

            if (!string.IsNullOrEmpty(item.jsonData))
            {
                // Base serialized fields such as node id and visual state are still present in the original payload.
                // Applying them to the placeholder lets edges keep pointing at the same node identity.
                JsonUtility.FromJsonOverwrite(item.jsonData, node);
            }

            return node;
        }

        private void SynthesizeMissingNodeSlotsFromEdges()
        {
            if (m_edges == null || m_edges.Count == 0 || m_nodes == null || m_nodes.Count == 0)
            {
                return;
            }

            List<MissingNode> missingNodes = GetNodesOfType<MissingNode>();
            ...
            {
                SynthesizeMissingNodeSlotsFromEdges(missingNode);
            }
        }

        private void SynthesizeMissingNodeSlotsFromEdges(MissingNode missingNode)
        {
            List<IEdge> connectedEdges = GetEdgesConnectedTo(missingNode.id);
            foreach (IEdge edge in connectedEdges)
            {
                if (edge.outputSlot.nodeId.Equals(missingNode.id))
                {
                    SynthesizeMissingNodeSlotFromEdge(missingNode, edge.outputSlot, SlotDirection.Output, edge.inputSlot);
                }

                if (edge.inputSlot.nodeId.Equals(missingNode.id))
                {
                    SynthesizeMissingNodeSlotFromEdge(missingNode, edge.inputSlot, SlotDirection.Input, edge.outputSlot);
                }
            }
        }

        private void SynthesizeMissingNodeSlotFromEdge(MissingNode missingNode, SlotRef thisSlotRef, SlotDirection thisDirection, SlotRef otherSlotRef)
        {
            Type slotType = GetSyntheticSlotType(otherSlotRef);
            missingNode.AddSyntheticSlot(slotType, thisDirection, thisSlotRef.slotId);
        }

        private Type GetSyntheticSlotType(SlotRef otherSlotRef)
        {
            INode oppositeNode = GetNode(otherSlotRef.nodeId);
            if (oppositeNode == null)
            {
                return typeof(MaskSlot);
            }

            ISlot oppositeSlot = oppositeNode.GetSlot(otherSlotRef.slotId);
            if (oppositeSlot == null)
            {
                return typeof(MaskSlot);
            }

            return oppositeSlot.GetType();
        }
        ...
        /// <summary>
        /// Throws when two generic output nodes use the same non-empty output name.
        /// </summary>
        /// <exception cref="System.InvalidOperationException">
        /// Thrown when a duplicate generic output name is found.
        /// </exception>
        internal void RequireUniqueGenericOutputNames()
        {
            HashSet<string> outputNames = new HashSet<string>(System.StringComparer.Ordinal);
            List<OutputNode> outputNodes = GetNodesOfType<OutputNode>();
            for (int i = 0; i < outputNodes.Count; ++i)
            {
                outputNodes[i].RequireValidOutputName();
                string outputName = outputNodes[i].outputName;
                ...
                if (!outputNames.Add(outputName))
                {
                    throw new System.InvalidOperationException(
                        $"Graph '{name}' contains more than one generic output named '{outputName}'. Generic output names must be unique.");
                }
            }
        }
        ...
        protected List<IEdge> GetEdgesConnectedTo(string nodeId)
        {
            List<IEdge> result = new List<IEdge>();
            for (int i = 0; i < m_edges.Count; ++i)
            {
                IEdge edge = m_edges[i];
                if (edge.inputSlot.nodeId.Equals(nodeId) || edge.outputSlot.nodeId.Equals(nodeId))
                {
                    result.Add(edge);
                }
            }
