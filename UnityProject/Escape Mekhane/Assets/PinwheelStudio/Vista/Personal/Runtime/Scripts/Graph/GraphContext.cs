...
namespace Pinwheel.Vista.Graph
{
    ...
    public struct GraphContext
    {
        ...
        /// </param>
        /// <param name="transientResources">
        /// Execution-owned registry shared by every copy of this context.
        /// </param>
        /// <param name="cancellation">
        /// Read-only cancellation state shared with this execution and its subgraphs.
        /// </param>
        ...
        /// </exception>
        internal GraphContext(GraphAsset graph, string[] nodeIds, DataPool pool, IDictionary<int, Args> args, ExecutionProgress progress, GraphExecutionCache cache, TransientResourceRegistry transientResources, CancellationSignal cancellation)
        {
            m_graph = graph;
            ...
            m_dataPool = pool;
            m_transientResources = transientResources;
            m_cancellationSignal = cancellation;
            if (args != null)
            {
        ...
        /// <summary>
        /// Gets whether cancellation has been requested for the current graph execution.
        /// </summary>
        /// <remarks>
        /// Long-running nodes should check this value immediately after their existing yield points
        /// and exit with <c>yield break</c>. Nodes must not perform cancellation cleanup themselves;
        /// execution-owned cleanup is handled by the graph.
        /// </remarks>
        ...
        /// <summary>
        /// Registers a Unity object created by a node for automatic destruction when this graph execution ends.
        /// </summary>
        /// <param name="resource">The execution-owned Unity object to register.</param>
        /// <remarks>
        /// Call this immediately after creating a temporary Unity object owned by the current
        /// execution, such as a material, compute-shader clone, or generated texture. Registration
        /// transfers ownership to the graph; the node must not destroy the object afterward. The
        /// object remains available for the rest of the execution and is destroyed automatically on
        /// completion or when the execution handle is disposed. Do not register imported assets,
        /// shared or cached objects, pooled resources, or objects owned by another system.
        /// </remarks>
        /// <exception cref="System.ArgumentNullException">
        /// <paramref name="resource"/> is null or has already been destroyed.
        /// </exception>
        /// <exception cref="System.ObjectDisposedException">
        /// The execution's transient-resource registry has already been disposed.
        /// </exception>
        public void RegisterDestroyLater(UnityEngine.Object resource)
        {
            m_transientResources.RegisterDestroyLater(resource);
        }

        /// <summary>
        /// Registers an asset acquired through <see cref="Resources.Load"/> for automatic unloading
        /// when this graph execution ends.
        /// </summary>
        /// <param name="resource">The loaded asset to unload later.</param>
        /// <remarks>
        /// Call this immediately after loading an execution-owned asset from Resources. Registration
        /// transfers unload responsibility to the graph, so the node must not call
        /// <see cref="Resources.UnloadAsset"/> afterward. Do not use this for instantiated or generated
        /// objects; register those with <see cref="RegisterDestroyLater(UnityEngine.Object)"/>.
        /// </remarks>
        public void RegisterUnloadLater(UnityEngine.Object resource)
        {
            m_transientResources.RegisterUnloadLater(resource);
        }

        /// <summary>
        /// Registers a node-owned disposable object for automatic disposal when this graph execution ends.
        /// </summary>
        /// <param name="resource">The execution-owned disposable object to register.</param>
        /// <remarks>
        /// Use this overload for transient resources that already implement
        /// <see cref="System.IDisposable"/> or for a node-defined disposable bundle that releases
        /// several related resources. Register it immediately after creation. Registration transfers
        /// ownership to the graph, so the node must not call <see cref="System.IDisposable.Dispose"/>
        /// afterward. Resources are disposed in reverse registration order on completion or when the
        /// execution handle is disposed. Do not register borrowed, shared, cached, or pooled resources.
        /// </remarks>
        /// <exception cref="System.ArgumentNullException"><paramref name="resource"/> is null.</exception>
        /// <exception cref="System.ObjectDisposedException">The execution's transient-resource registry has already been disposed.</exception>
        public void RegisterDisposeLater(System.IDisposable resource)
        {
            m_transientResources.RegisterDisposeLater(resource);
        }
        ...
        {
            return graph.Execute(nodeIds, configs, inputContainer, fillArgumentsCallback, m_cache, m_cancellationSignal);
        }

        private string GetCacheKey(string nodeId)
        {
            int graphId = Utilities.GetObjectIdHash(m_graph);
        ...
        /// <summary>
        /// Returns whether a valid external texture has been registered under the supplied name.
        /// </summary>
        public bool HasExternalTexture(string name)
        {
            if (string.IsNullOrEmpty(name))
                return false;

            return m_externalTextures.TryGetValue(name, out RenderTexture texture) && texture != null;
        }
