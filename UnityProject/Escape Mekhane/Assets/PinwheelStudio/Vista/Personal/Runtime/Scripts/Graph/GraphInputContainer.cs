...
namespace Pinwheel.Vista.Graph
{
    ...
    public class GraphInputContainer
    {
        ...
        public void AddTexture(string name, RenderTexture texture)
        {
            ...
            {
                Debug.LogError($"[Vista] A graph texture input named '{name}' has already been registered in this input container.");
                throw;
            }
        }
        ...
        public void AddBuffer(string name, ComputeBuffer buffer)
        {
            ...
            {
                Debug.LogError($"[Vista] A graph buffer input named '{name}' has already been registered in this input container.");
                throw;
            }
        }

        /// <summary>Returns a registered texture without transferring ownership.</summary>
        public RenderTexture GetTexture(string name)
        {
            m_textures.TryGetValue(name, out RenderTexture texture);
            return texture;
        }
