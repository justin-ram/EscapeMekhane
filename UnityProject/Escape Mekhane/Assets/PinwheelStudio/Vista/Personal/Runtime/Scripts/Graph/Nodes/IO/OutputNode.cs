...
namespace Pinwheel.Vista.Graph
{
    ...
    public class OutputNode : ExecutableNodeBase, IOutputNode, IHasDynamicSlotCount, ISerializationCallbackReceiver
    {
        ...
        public string outputName
        {
            ...
            set
            {
                if (!IsOutputNameAllowed(value))
                {
                    throw new ArgumentException(
                        $"[Vista] '{GraphConstants.SCENE_HEIGHT_INPUT_NAME}' is reserved for graph input and cannot be used as an output name.",
                        nameof(value));
                }
                m_outputName = value;
            }
        ...
        public static bool IsOutputNameAllowed(string outputName)
        {
            return !string.Equals(outputName, GraphConstants.SCENE_HEIGHT_INPUT_NAME, StringComparison.Ordinal);
        }

        internal void RequireValidOutputName()
        {
            if (!IsOutputNameAllowed(m_outputName))
            {
