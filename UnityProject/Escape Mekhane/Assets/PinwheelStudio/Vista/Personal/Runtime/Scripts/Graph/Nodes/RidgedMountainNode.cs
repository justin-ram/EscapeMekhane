...
namespace Pinwheel.Vista.Graph
{
    [NodeMetadata(
        ...
        path = "Legacy/Generators/Ridged Mountain",
    ...
    public class RidgedMountainNode : ImageNodeBase
    {
        ...
        public override void ExecuteImmediate(GraphContext context)
        {
            ...
            NodeLibraryUtilities.NoiseNode.Execute(context, tempRt0, p);
            ...
            NodeLibraryUtilities.ClampNode.Execute(context, tempRt0, tempRt1, 0f, 1f);

            RenderTexture tempRt2 = context.CreateTemporaryRT(desc1, TEMP_RT2_NAME);
            NodeLibraryUtilities.ConvexNode.Execute(context, tempRt1, tempRt2, 0, 3);

            int dfIteration = Mathf.RoundToInt(desc1.width / 10f);
            NodeLibraryUtilities.DistanceFieldNode.Execute(context, tempRt2, tempRt1, dfIteration);
            ...
            NodeLibraryUtilities.CombineNode.Execute(context, tempRt0, tempRt1, Texture2D.whiteTexture, combineMode, tempRt3);

            RenderTexture tempRt4 = context.CreateTemporaryRT(desc, TEMP_RT4_NAME);
            combineMode = NodeLibraryUtilities.CombineNode.MODE_ADD;
            NodeLibraryUtilities.CombineNode.Execute(context, tempRt0, tempRt3, Texture2D.whiteTexture, combineMode, tempRt4, 1.0f, 0.1f, 1.0f);

            NodeLibraryUtilities.BlurNode.Execute(context, tempRt4, tempRt0, 5);
