...
namespace Pinwheel.Vista.Graph
{
    ...
    public class Transform2dNode : ImageNodeBase, ISerializationCallbackReceiver, IHasDynamicSlotCount
    {
        ...
        public static readonly int TILE_X = Shader.PropertyToID("_TileX");
        public static readonly int TILE_Y = Shader.PropertyToID("_TileY");
        ...
        public override void ExecuteImmediate(GraphContext context)
        {
            ...
            {
                m_material.SetInteger(TILE_X, 1);
            }
            else
            {
                m_material.SetInteger(TILE_X, 0);
            }

            if (tilingMode == TilingMode.TileY || tilingMode == TilingMode.TileXY)
            {
                m_material.SetInteger(TILE_Y, 1);
            }
            else
            {
                m_material.SetInt(TILE_Y, 0);
            }
