...
namespace Pinwheel.Vista.Graph
{
    ...
    {
        /// <summary>
        /// Gives a host the opportunity to claim ownership of a generic texture after it is removed from the execution pool.
        /// Return true when the host claims the texture; otherwise it is added to the request's generic outputs.
        /// </summary>
        public delegate bool GenericTextureOutputHandler(string name, RenderTexture texture);

        /// <summary>
        /// Gives a host the opportunity to claim ownership of a generic buffer after it is removed from the execution pool.
        /// Return true when the host claims the buffer; otherwise it is added to the request's generic outputs.
        /// </summary>
        public delegate bool GenericBufferOutputHandler(string name, ComputeBuffer buffer);

        /// <summary>
        /// Executes a terrain graph for one biome request and fills the requested output channels into <see cref="BiomeData"/>.
        /// </summary>
        /// <param name="biome">
        /// The biome requesting data. This is used for progress reporting and for identifying
        ...
        /// Optional read-only cancellation state passed to <see cref="TerrainGraph.Execute"/>. When cancellation is observed,
        /// the method waits for graph cleanup, disposes the execution handle without transferring any pooled outputs, and
        /// completes <paramref name="request"/> with its existing payload unchanged.
        /// </param>
        /// <returns>
        ...
        /// </para>
        /// <para>
        /// Cancellation is checked before output extraction. This is the ownership boundary: a cancelled execution retains
        /// and disposes every pooled output, while a successful execution transfers selected outputs to the request.
        /// </para>
        /// <para>
        ...
        public static IEnumerator RequestBiomeData(IBiome biome, BiomeDataRequest request, TerrainGraph graph, Bounds biomeWorldBounds, Space simSpace, int baseResolution, int seed, GraphInputContainer inputContainer = null, BiomeDataMask dataMask = (BiomeDataMask)~0, TerrainDataGraph.FillArgumentsHandler fillArgumentsCallback = null, GraphExecutionCache cache = null, CancellationSignal cancellationSignal = null)
        {
            ...
            IEnumerator requestRoutine = RequestData(
                $"Processing biome {biome.gameObject.name}",
                request,
                graph,
                configs,
                inputContainer,
                dataMask,
                fillArgumentsCallback,
                cache,
                cancellationSignal);
        ...
        /// <summary>
        /// Executes a terrain graph for one request against explicit generation configs and fills the requested
        /// output channels into <see cref="BiomeData"/>.
        /// </summary>
        /// <param name="progressLabel">Text published while the graph runs.</param>
        /// <param name="request">
        /// The asynchronous request whose <see cref="BiomeDataRequest.data"/> instance will receive the generated outputs.
        /// </param>
        /// <param name="graph">The terrain graph to execute.</param>
        /// <param name="configs">
        /// Resolution, world bounds, terrain height, and seed for this run, already resolved by the caller.
        /// </param>
        /// <param name="inputContainer">Optional external graph inputs.</param>
        /// <param name="dataMask">Bitmask selecting which output categories are requested and extracted.</param>
        /// <param name="fillArgumentsCallback">Optional callback that appends extra execution arguments.</param>
        /// <param name="cache">Optional execution cache reused across runs of the same graph.</param>
        /// <param name="cancellationSignal">Optional cooperative cancellation signal.</param>
        /// <param name="genericTextureOutputHandler">
        /// Optional ownership hook invoked after each generic texture is removed from the execution pool. Returning
        /// true transfers the texture to the host instead of adding it to <paramref name="request"/>.
        /// </param>
        /// <remarks>
        /// This is the shared body behind <see cref="RequestBiomeData"/>. Hosts that are not biomes, such as the Stroke
        /// painter, resolve their own configs and call this directly. Output ownership rules are identical: a cancelled
        /// execution disposes every pooled output, a successful one transfers the selected outputs to the request.
        /// </remarks>
        public static IEnumerator RequestData(string progressLabel, BiomeDataRequest request, TerrainDataGraph graph, TerrainGenerationConfigs configs, GraphInputContainer inputContainer = null, BiomeDataMask dataMask = (BiomeDataMask)~0, TerrainDataGraph.FillArgumentsHandler fillArgumentsCallback = null, GraphExecutionCache cache = null, CancellationSignal cancellationSignal = null, GenericTextureOutputHandler genericTextureOutputHandler = null, GenericBufferOutputHandler genericBufferOutputHandler = null)
        {
            if (configs.resolution < 0 || configs.resolution % 8 != 0)
            {
                throw new System.ArgumentException("Invalid base resolution, must be 8*x");
            }
            ...
            List<TextureOutputNodeBase> textureOutputNodes = null;
            if (dataMask.HasFlag(BiomeDataMask.LayerWeightMaps))
            {
                textureOutputNodes = graph.GetNodesOfType<TextureOutputNodeBase>();
                textureOutputNodes.RemoveAll(n => n.terrainLayer == null);
                textureOutputNodes.Sort((n0, n1) => { return n0.order.CompareTo(n1.order); });
            }
            else
            {
                textureOutputNodes = new List<TextureOutputNodeBase>();
            }
            foreach (TextureOutputNodeBase n in textureOutputNodes)
            {
                nodeIds.Add(n.id);
            }

            List<TreeOutputNodeBase> treeOutputNodes = null;
            if (dataMask.HasFlag(BiomeDataMask.TreeInstances))
            {
                treeOutputNodes = graph.GetNodesOfType<TreeOutputNodeBase>();
                treeOutputNodes.RemoveAll(n => n.treeTemplate == null || !n.treeTemplate.IsValid());
            }
            else
            {
                treeOutputNodes = new List<TreeOutputNodeBase>();
            }
            foreach (TreeOutputNodeBase n in treeOutputNodes)
            {
                nodeIds.Add(n.id);
            }

            List<DetailDensityOutputNodeBase> detailDensityOutputNodes = null;
            if (dataMask.HasFlag(BiomeDataMask.DetailDensityMaps))
            {
                detailDensityOutputNodes = graph.GetNodesOfType<DetailDensityOutputNodeBase>();
                detailDensityOutputNodes.RemoveAll(n => n.detailTemplate == null || !n.detailTemplate.IsValid());
            }
            else
            {
                detailDensityOutputNodes = new List<DetailDensityOutputNodeBase>();
            }
            foreach (DetailDensityOutputNodeBase n in detailDensityOutputNodes)
            {
                nodeIds.Add(n.id);
            }

            List<DetailInstanceOutputNodeBase> detailInstanceOutputNodes = null;
            if (dataMask.HasFlag(BiomeDataMask.DetailInstances))
            {
                detailInstanceOutputNodes = graph.GetNodesOfType<DetailInstanceOutputNodeBase>();
                detailInstanceOutputNodes.RemoveAll(n => n.detailTemplate == null || !n.detailTemplate.IsValid());
            }
            else
            {
                detailInstanceOutputNodes = new List<DetailInstanceOutputNodeBase>();
            }
            foreach (DetailInstanceOutputNodeBase n in detailInstanceOutputNodes)
            {
                nodeIds.Add(n.id);
            }
            ...
            System.Guid progressId = ProgressNotifications.Begin(progressLabel);
            try
            {
                ExecutionHandle handle = graph.Execute(nodeIds.ToArray(), configs, inputContainer, fillArgumentsCallback, cache, cancellationSignal);
                while (!handle.isCompleted)
                {
                    ProgressNotifications.Report(progressId, handle.progress.totalProgress);
                    yield return null;
                }

                if (handle.isFaulted)
                {
                    System.Exception failure = handle.exception;
                    handle.Dispose();
                    throw failure;
                }
                ...
                foreach (OutputNode n in genericOutputNodes)
                {
                    if (n.slotType.Equals(typeof(BufferSlot)))
                    {
                        if (dataMask.HasFlag(BiomeDataMask.GenericBuffers))
                        {
                            ...
                            {
                                bool isClaimed = genericBufferOutputHandler != null &&
                                    genericBufferOutputHandler.Invoke(n.outputName, buffer.buffer);
                                if (!isClaimed)
                                {
                    ...
                    else if (n.slotType.Equals(typeof(MaskSlot)) || n.slotType.Equals(typeof(ColorTextureSlot)))
                    {
                        if (dataMask.HasFlag(BiomeDataMask.GenericTextures))
                        {
                            ...
                            {
                                bool isClaimed = genericTextureOutputHandler != null &&
                                    genericTextureOutputHandler.Invoke(n.outputName, generatedTexture);
                                if (!isClaimed)
                                {
        ...
        public static bool HasSceneHeightInput(this TerrainDataGraph graph)
        {
