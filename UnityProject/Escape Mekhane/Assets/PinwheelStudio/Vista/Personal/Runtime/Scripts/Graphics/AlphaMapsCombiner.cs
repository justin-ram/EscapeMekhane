...
namespace Pinwheel.Vista.Graphics
{
    ...
    public class AlphaMapsCombiner
    {
        ...
        private static readonly int ALPHA_MAP = Shader.PropertyToID("_AlphaMap");
        ...
        private static readonly int PASS_EXTRACT_CHANNEL = 1;
        ...
        /// <summary>
        /// Extracts one layer weight from a packed RGBA alphamap texture.
        /// </summary>
        /// <param name="alphaMap">Packed source alphamap.</param>
        /// <param name="channelIndex">RGBA channel index in the range zero through three.</param>
        /// <param name="resolution">Resolution of the output texture.</param>
        /// <returns>A caller-owned single-channel weight texture.</returns>
        public RenderTexture ExtractChannel(Texture alphaMap, int channelIndex, int resolution)
        {
            if (resolution <= 0)
            {
            ...
            RenderTexture layerWeight = GraphicsUtils.CreateBlankRT(resolution, RenderTextureFormat.RFloat);
            try
            {
                ExtractChannel(alphaMap, channelIndex, layerWeight, Drawing.unitQuad);
                return layerWeight;
            }
            catch
            {
                layerWeight.Release();
                Object.DestroyImmediate(layerWeight);
                throw;
            }
        }

        /// <summary>
        /// Extracts one layer weight from a packed RGBA alphamap directly into a region of an existing render texture.
        /// </summary>
        public void ExtractChannel(Texture alphaMap, int channelIndex, RenderTexture target, Vector2[] quadCorners)
        {
            if (alphaMap == null)
            {
                throw new System.ArgumentNullException(nameof(alphaMap));
            }
            if (channelIndex < 0 || channelIndex > 3)
            {
                throw new System.ArgumentOutOfRangeException(nameof(channelIndex));
            }
            if (target == null)
            {
            ...
            {
                throw new System.ArgumentException("A texture extraction quad must contain exactly four corners.", nameof(quadCorners));
            }
            ...
            {
                Vector4 channelMask = Vector4.zero;
                channelMask[channelIndex] = 1;
                mat.SetTexture(ALPHA_MAP, alphaMap);
                mat.SetVector(CHANNEL_MASK, channelMask);
                Drawing.DrawQuad(target, quadCorners, mat, PASS_EXTRACT_CHANNEL);
            }
            finally
            {
                Object.DestroyImmediate(mat);
