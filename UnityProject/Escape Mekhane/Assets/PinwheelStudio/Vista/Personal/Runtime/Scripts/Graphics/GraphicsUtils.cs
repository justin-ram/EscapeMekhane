...
namespace Pinwheel.Vista
{
    ...
    public static class GraphicsUtils
    {
        ...
        /// <summary>
        /// Creates a float-packed compute buffer for Vista sample data.
        /// </summary>
        /// <typeparam name="T">Either <see cref="InstanceSample"/> or <see cref="PositionSample"/>.</typeparam>
        /// <param name="sampleCount">Number of samples the buffer must contain.</param>
        /// <returns>A compute buffer sized using the float count of the requested sample type.</returns>
        public static ComputeBuffer CreateSampleBuffer<T>(int sampleCount)
        {
            if (sampleCount <= 0)
            {
                throw new System.ArgumentOutOfRangeException(nameof(sampleCount));
            }

            int sampleSize;
            if (typeof(T) == typeof(InstanceSample))
            {
                sampleSize = InstanceSample.SIZE;
            }
            else if (typeof(T) == typeof(PositionSample))
            {
                sampleSize = PositionSample.SIZE;
            }
            else
            {
                throw new System.ArgumentException($"Unsupported sample type: {typeof(T).Name}.", nameof(T));
            }

            return new ComputeBuffer(sampleCount * sampleSize, sizeof(float));
        }
        ...
        /// </summary>
        /// <param name="uploadToGpu">Whether to upload the read pixels back to the Texture2D's GPU resource.</param>
        public static void ReadRenderTexture(RenderTexture source, Texture2D destination, bool uploadToGpu = true)
        {
            RenderTexture previousActive = RenderTexture.active;
