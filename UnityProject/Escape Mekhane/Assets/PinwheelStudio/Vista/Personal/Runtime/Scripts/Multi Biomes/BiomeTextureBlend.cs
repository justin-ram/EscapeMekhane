...
namespace Pinwheel.Vista.BigWorld
{
    ...
    internal static class BiomeTextureBlend
    {
        ...
        /// <summary>
        /// Replaces values in <paramref name="dest"/> with <paramref name="src"/>, controlled by <paramref name="mask"/>.
        /// </summary>
        internal static void BlendReplace(
            Texture src,
            RenderTexture dest,
            Texture mask,
            BlendContext ctx,
            float srcRemapOffset = 0f,
            float srcRemapScale = 1f)
        {
            DispatchBlend(
                ctx.shader,
                src,
        ...
        /// <param name="configs">Ordered source blend settings.</param>
        /// <param name="srcDatas">Biome output data parallel to <paramref name="configs"/>.</param>
        ...
        internal static void BlendHeightMap(
            BiomeData destData, List<BiomeBlendConfig> configs, List<BiomeData> srcDatas, BlendContext ctx)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < srcDatas.Count; ++i)
                {
                    ...
                    BiomeBlendConfig config = configs[i];
                    BiomeBlendOptions.HeightBlendMode blendMode = config.blendOptions.heightMapBlendMode;
                    ...
                    float normalizedOffset = config.heightOffset;
                    float normalizedScale = config.heightScale;

                    if (config.blendOptions.textureBlendMode == BiomeBlendOptions.TextureBlendMode.HeightWin ||
                        config.blendOptions.populationBlendMode == BiomeBlendOptions.PopulationBlendMode.HeightWin)
                    {
            ...
            BiomeData destData, List<BiomeBlendConfig> configs, List<BiomeData> srcDatas, BlendContext ctx)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < srcDatas.Count; ++i)
                {
                    BiomeData srcData = srcDatas[i];
                    Texture mask = srcData.biomeMaskMap != null ? (Texture)srcData.biomeMaskMap : Texture2D.whiteTexture;
                    BiomeBlendOptions.HoleBlendMode blendMode = configs[i].blendOptions.holeMapBlendMode;
            ...
            BiomeData destData, List<BiomeBlendConfig> configs, List<BiomeData> srcDatas, BlendContext ctx)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < srcDatas.Count; ++i)
                {
                    BiomeData srcData = srcDatas[i];
                    Texture mask = srcData.biomeMaskMap != null ? (Texture)srcData.biomeMaskMap : Texture2D.whiteTexture;
                    BiomeBlendOptions.MeshDensityBlendMode blendMode = configs[i].blendOptions.meshDensityBlendMode;
            ...
            BiomeData destData, List<BiomeBlendConfig> configs, List<BiomeData> srcDatas, BlendContext ctx)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < srcDatas.Count; ++i)
                {
                    ...
                    BiomeBlendConfig config = configs[i];
                    if (config.blendOptions.textureBlendMode == BiomeBlendOptions.TextureBlendMode.Replace)
                    {
                        DispatchBlend(ctx.shader, srcMap, dest, biomeMask, linearKernel);
                    }
                    else if (config.blendOptions.textureBlendMode == BiomeBlendOptions.TextureBlendMode.HeightWin)
                    {
                        Texture heightWinMask = ctx.GetHeightWinMask(i);
                        if (heightWinMask == null)
                        {
                            Debug.Log(
                                $"[Biome Blend/Albedo] Source '{config.label}' uses Height Win texture blending, but it has no height output. " +
                    ...
                    else
                    {
                        throw new System.NotImplementedException(
                            $"No albedo blend implementation for TextureBlendMode.{config.blendOptions.textureBlendMode}. Add a case here.");
                    }
                }
            ...
            else
            {
                ...
                for (int i = 0; i < srcDatas.Count; ++i)
                {
                    ...
                    else if (config.blendOptions.textureBlendMode == BiomeBlendOptions.TextureBlendMode.HeightWin)
                    {
                        ...
                        {
                            Debug.Log(
                                $"[Biome Blend/Metallic] Source '{config.label}' uses Height Win texture blending, but it has no height output. " +
                                $"Enable Height in the Data Mask and add a Height Output node.");
                            continue;
                        }
                    ...
                    {
                        throw new System.NotImplementedException(
                            $"No metallic blend implementation for TextureBlendMode.{config.blendOptions.textureBlendMode}. Add a case here.");
                    }
                }
            ...
            for (int iLabel = 0; iLabel < uniqueLabels.Count; ++iLabel)
            {
                ...
                for (int i = 0; i < srcDatas.Count; ++i)
                {
                    ...
                    else if (config.blendOptions.textureBlendMode == BiomeBlendOptions.TextureBlendMode.HeightWin)
                    {
                        ...
                        {
                            Debug.Log(
                                $"[Biome Blend/Generic Texture] Source '{config.label}' uses Height Win texture blending, but it has no height output. " +
                                $"Enable Height in the Data Mask and add a Height Output node.");
                            continue;
                        }
                    ...
                    {
                        throw new System.NotImplementedException(
                            $"No generic texture blend implementation for TextureBlendMode.{config.blendOptions.textureBlendMode}. Add a case here.");
                    }
                }
        ...
        /// <param name="ctx">Texture blend context created by <see cref="Begin"/>.</param>
        /// <remarks>
        /// Texture layers are accumulated as non-distinct layer/weight pairs. Each biome erodes weights accumulated from
        /// lower biomes once using its effective coverage, then adds all of its own weights using the biome mask. This
        /// preserves the source biome's internal weight distribution. Biomes with no layers or no biome mask contribute
        /// black coverage.
        /// </remarks>
        /// <remarks>
            ...
            BiomeData destData, List<BiomeBlendConfig> configs, List<BiomeData> srcDatas, BlendContext ctx)
        {
            ...
            if (hasContribution)
            {
                ...
                List<Texture> heightWinMasksByBiome = new List<Texture>();
                for (int iBiome = 0; iBiome < configs.Count; ++iBiome)
                {
                    Texture heightWinMask = null;
                    if (layerCountsByBiome[iBiome] > 0 &&
                        configs[iBiome].blendOptions.textureBlendMode == BiomeBlendOptions.TextureBlendMode.HeightWin)
                    {
                        heightWinMask = ctx.GetHeightWinMask(iBiome);
                        if (heightWinMask == null)
                        {
                            Debug.Log(
                                $"[Biome Blend/Texture Weights] Source '{configs[iBiome].label}' uses Height Win texture blending, but it has no height output. " +
                                $"Enable Height in the Data Mask and add a Height Output node.");
                            heightWinMask = Texture2D.blackTexture;
                        }
                    }
                    heightWinMasksByBiome.Add(heightWinMask);
                }
                ...
                for (int iWeight = 0; iWeight < srcWeights.Count; ++iWeight)
                {
                    ...
                    BiomeBlendConfig config = configs[biomeIndex];

                    if (config.blendOptions.textureBlendMode == BiomeBlendOptions.TextureBlendMode.HeightWin)
                    {
                        Texture heightWinMask = heightWinMasksByBiome[biomeIndex];
                ...
                for (int iBiome = 0; iBiome < configs.Count; ++iBiome)
                {
                    int layerCount = layerCountsByBiome[iBiome];
                    if (layerCount <= 0)
                        continue;

                    Texture biomeMask = srcBiomeMasks[iBiome];
                    Texture coverageMask = biomeMask;
                    RenderTexture heightWinCoverageMask = null;
                    BiomeBlendOptions.TextureBlendMode blendMode = configs[iBiome].blendOptions.textureBlendMode;
                    if (blendMode == BiomeBlendOptions.TextureBlendMode.HeightWin)
                    {
                        // Incoming weights are already height-filtered; combine masks only for lower-layer erosion.
                        heightWinCoverageMask = GraphicsUtils.GetBlankTempRT(resolution, format);
                        FilterBiomeMaskWithHeightWinMask(
                            ctx, biomeMask, heightWinMasksByBiome[iBiome], heightWinCoverageMask);
                        coverageMask = heightWinCoverageMask;
                    }
                    else if (blendMode != BiomeBlendOptions.TextureBlendMode.Replace)
                    {
                        throw new System.NotImplementedException(
                            $"No texture-weight blend implementation for TextureBlendMode.{blendMode}. Add a case here.");
                    }

                    // The incoming biome claims its coverage once. Its sibling texture layers must not erode each other.
                    for (int iPreviousWeight = 0; iPreviousWeight < layerOffset; ++iPreviousWeight)
                    {
                        ErodeCoverageMask(ctx, destWeights[iPreviousWeight], coverageMask);
                    }

                    for (int iLayer = 0; iLayer < layerCount; ++iLayer)
                    {
                        int weightIndex = layerOffset + iLayer;
                        DispatchBlend(
                            ctx.shader,
                            filteredWeights[weightIndex],
                            destWeights[weightIndex],
                            biomeMask,
                            linearKernel);
                    }

                    if (heightWinCoverageMask != null)
                    {
                        GraphicsUtils.ReleaseTempRT(heightWinCoverageMask);
                    }
            ...
            BiomeData destData, List<BiomeBlendConfig> configs, List<BiomeData> srcDatas, BlendContext ctx)
        {
            ...
            for (int iBiome = 0; iBiome < srcDatas.Count; ++iBiome)
            {
                BiomeBlendConfig config = configs[iBiome];
                ...
                VistaDebugger.CaptureString("Source", $"{config.label} | {config.blendOptions.populationBlendMode}");
                VistaDebugger.CaptureTexture("Biome Mask", biomeMask);
                VistaDebugger.CaptureTexture("Src Collection", srcDensityMaps);

                if (config.blendOptions.populationBlendMode == BiomeBlendOptions.PopulationBlendMode.Replace)
                {
                ...
                else if (config.blendOptions.populationBlendMode == BiomeBlendOptions.PopulationBlendMode.Coexist)
                {
                    // Coexist only adds this biome's masked density maps without cleaning underneath.
                    AppendMaskedDensityMaps(
                        ctx, srcDetailTemplates, srcDensityMaps, biomeMask, destDetailTemplates, destDensityMaps);
                }
                else if (config.blendOptions.populationBlendMode == BiomeBlendOptions.PopulationBlendMode.HeightWin)
                {
                    ...
                    {
                        Debug.Log(
                            $"[Biome Blend/Detail Density] Source '{config.label}' uses Height Win population blending, but it has no height output. Enable Height in the Data Mask and add a Height Output node.");
                    }
                }
                else
                {
                    throw new System.NotImplementedException(
                        $"No density map gather implementation for PopulationBlendMode.{config.blendOptions.populationBlendMode}. Add a case here.");
                }
