#if VISTA
using System.Collections;
using System;
#if UNITY_EDITOR
using Unity.EditorCoroutines.Editor;
#endif

namespace Pinwheel.Vista
{
    /// <summary>
    /// Starts and stops Vista coroutines through a single API that works in both the Unity editor and play mode.
    /// </summary>
    public class CoroutineUtility
    {
        private sealed class GuardState
        {
            public readonly Action<Exception> exceptionHandler;
            public bool isFaulted;

            public GuardState(Action<Exception> exceptionHandler)
            {
                this.exceptionHandler = exceptionHandler;
            }

            public void Fail(Exception exception)
            {
                if (isFaulted)
                    return;

                isFaulted = true;
                exceptionHandler.Invoke(exception);
            }
        }

        private static CoroutineManager manager { get; set; }

        /// <summary>
        /// Starts a coroutine and returns a handle that can later be passed back to <see cref="StopCoroutine(CoroutineHandle)"/>.
        /// </summary>
        /// <param name="coroutine">The enumerator to run.</param>
        /// <returns>
        /// A Vista coroutine handle that wraps either an editor coroutine or a runtime coroutine, depending on the current execution environment.
        /// </returns>
        /// <remarks>
        /// The backing <see cref="CoroutineManager"/> instance is created lazily the first time this method is called.
        /// </remarks>
        public static CoroutineHandle StartCoroutine(IEnumerator coroutine)
        {
            if (manager == null)
            {
                manager = CoroutineManager.CreateInstance();
            }

            CoroutineHandle handler = new CoroutineHandle();
            handler.manager = manager;
#if UNITY_EDITOR
            handler.coroutine = EditorCoroutineUtility.StartCoroutine(coroutine, manager);
#else
            handler.coroutine = manager.StartCoroutine(coroutine);
#endif
            return handler;
        }

        /// <summary>
        /// Starts a coroutine and reports exceptions from it or nested enumerators through the supplied handler.
        /// </summary>
        /// <param name="coroutine">The enumerator to run.</param>
        /// <param name="exceptionHandler">
        /// Called once when the coroutine or one of its nested enumerators throws. The coroutine stops after the callback.
        /// </param>
        /// <returns>A Vista coroutine handle that can be passed to <see cref="StopCoroutine(CoroutineHandle)"/>.</returns>
        public static CoroutineHandle StartCoroutine(
            IEnumerator coroutine,
            Action<Exception> exceptionHandler)
        {
            if (coroutine == null)
                throw new ArgumentNullException(nameof(coroutine));
            if (exceptionHandler == null)
                throw new ArgumentNullException(nameof(exceptionHandler));

            GuardState state = new GuardState(exceptionHandler);
            return StartCoroutine(GuardProgressive(coroutine, state));
        }

        private static IEnumerator GuardProgressive(IEnumerator coroutine, GuardState state)
        {
            try
            {
                while (!state.isFaulted)
                {
                    bool hasNext;
                    object current;
                    try
                    {
                        hasNext = coroutine.MoveNext();
                        current = hasNext ? coroutine.Current : null;
                    }
                    catch (Exception exception)
                    {
                        state.Fail(exception);
                        yield break;
                    }

                    if (!hasNext)
                        yield break;

                    if (current is IEnumerator nestedCoroutine)
                    {
                        yield return GuardProgressive(nestedCoroutine, state);
                    }
                    else
                    {
                        yield return current;
                    }
                }
            }
            finally
            {
                if (coroutine is IDisposable disposable)
                {
                    try
                    {
                        disposable.Dispose();
                    }
                    catch (Exception exception)
                    {
                        state.Fail(exception);
                    }
                }
            }
        }

        /// <summary>
        /// Stops a coroutine previously started through <see cref="StartCoroutine(IEnumerator)"/>.
        /// </summary>
        /// <param name="coroutine">The handle returned when the coroutine was started.</param>
        public static void StopCoroutine(CoroutineHandle coroutine)
        {
            if (coroutine.coroutine != null)
            {
#if UNITY_EDITOR
                EditorCoroutineUtility.StopCoroutine(coroutine.coroutine);
#else
                coroutine.manager.StopCoroutine(coroutine.coroutine);
#endif
            }
        }
    }
}
#endif


