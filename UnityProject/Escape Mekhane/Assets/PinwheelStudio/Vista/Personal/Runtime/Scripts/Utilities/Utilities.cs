...
namespace Pinwheel.Vista
{
    ...
    public static class Utilities
    {
        ...
        /// <summary>
        /// Resolution-bucketed iterations-per-frame for split simulation nodes. Keeps each frame's GPU batch
        /// small enough to stay under the driver's TDR watchdog (a flat high count crashes the editor at 4K)
        /// while amortizing per-frame overhead so large bakes stay responsive without being needlessly slow.
        /// Values are hand-tuned for safety and responsiveness and kept conservative so multiple nodes can share
        /// them; the ranges also cover non-power-of-two resolutions. 4096 is the maximum officially supported
        /// output; above that this returns 1 for safety (callers overriding to higher resolutions should disable
        /// the auto behavior and accept the TDR risk).
        /// </summary>
        /// <param name="resolution">Resolution of the stage being simulated, in pixels.</param>
        /// <returns>The number of simulation iterations to run per frame yield at that resolution.</returns>
        public static int GetAutoIterationPerFrame(int resolution)
        {
            if (resolution <= 512) return 64;
            if (resolution <= 1024) return 48;
            if (resolution <= 2048) return 32;
            if (resolution <= 3072) return 12;
            if (resolution <= 4096) return 8;
            return 1;
        }
        ...
        public static int GetObjectIdHash(Object obj)
        {
            if (obj == null)
            {
                return 0;
        ...
        public static T FindAnyObjectCompat<T>(bool includeInactive = false) where T : Object
        {
        ...
        public static T[] FindObjectsNoSortCompat<T>(bool includeInactive = false) where T : Object
        {
#if UNITY_6000_4_OR_NEWER
            if (includeInactive)
            {
...
#if UNITY_EDITOR
        public static Object EditorIdToObjectCompat(int instanceId)
        {
#if UNITY_6000_3_OR_NEWER
            // Unity 6.3+ deprecates InstanceIDToObject in favor of EntityIdToObject.
            // The editor callback still gives us an int instance id here, so keep the conversion centralized.
...
#pragma warning restore CS0618
        ...
        public static void DrawMeshCompat(
            Mesh mesh,
            Matrix4x4 matrix,
            Material material,
            int layer,
            ...
            UnityEngine.Rendering.LightProbeUsage lightProbeUsage)
        {
            ...
            UnityEngine.Graphics.DrawMesh(
                mesh, matrix, material, layer, camera, submeshIndex, properties,
                castShadows, receiveShadows, probeAnchor, lightProbeUsage);
#else
            UnityEngine.Graphics.DrawMesh(
                mesh, matrix, material, layer, camera, submeshIndex, properties,
                castShadows, receiveShadows, probeAnchor, lightProbeUsage, null);
#endif
        }
        ...
        /// <summary>
        /// Tests whether two bounds overlap on the horizontal XZ plane, ignoring their Y extents.
        /// </summary>
        public static bool OverlapXZ(Bounds a, Bounds b)
        {
        ...
        /// <summary>
        /// Destroys a Unity object using the operation appropriate for the current play mode.
        /// </summary>
